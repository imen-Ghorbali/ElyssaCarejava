...
public class service_consultation implements IService_consultation<consultation> {
    ...
    public List<consultation> getConsultationsByUser(int userId) {
        List<consultation> consultations = new ArrayList<>();
        try {
            String query = "SELECT * FROM consultation WHERE patient = ?"; // Changé user_id en patient
            PreparedStatement statement = cnx.prepareStatement(query);
            statement.setInt(1, userId); // Utilisation de PreparedStatement pour éviter les injections SQL
            ResultSet rs = statement.executeQuery();

            while (rs.next()) {
                consultation c = new consultation();
                c.setId(rs.getInt("id"));
                c.setDate(rs.getTimestamp("date").toLocalDateTime()); // Correction pour utiliser Timestamp
                c.setStatus(rs.getString("status"));

                // Ajout des autres informations nécessaires
                c.setPatient(getUserById(rs.getInt("patient")));
                c.setDoctor(getUserById(rs.getInt("doctor")));
                c.setSpecialite(type_specialite.valueOf(rs.getString("specialite")));
    ...
    public ArrayList<consultation> getConsultationsByDoctor(int doctorId) {
        ArrayList<consultation> list = new ArrayList<>();
        String req = "SELECT * FROM consultation WHERE doctor = ?";
        try {
            PreparedStatement pst = cnx.prepareStatement(req);
            ...
            System.err.println("❌ Erreur récupération consultations du docteur : " + e.getMessage());
        }
        return list;
    }

...
public class addConsultation {
    ...
    void addConsultation(ActionEvent event) {
        try {
            ...
            // Contrôle de saisie pour le status
            if (status.trim().isEmpty() || !status.matches("[a-zA-Z ]+")) {
                showAlert(Alert.AlertType.WARNING, "Statut invalide", "Le statut ne doit contenir que des lettres.");
                return;
            }

            // Vérification que l'heure choisie n'est pas dans le passé
            if (tf_time.getValue() != null && tf_time.getValue().isBefore(LocalDateTime.now().toLocalDate())) {
                showAlert(Alert.AlertType.WARNING, "Heure invalide", "L'heure ne peut pas être dans le passé.");
                return;
            }

            if (selectedDoctor == null || specialite == null || tf_time.getValue() == null) {

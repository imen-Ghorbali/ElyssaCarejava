...
public class inscription {
   ...
   /* @FXML
    void ajouter(ActionEvent event) {
        ...
        if (name.trim().isEmpty() || email.trim().isEmpty() || password.trim().isEmpty() || role == null) {
            showAlert(Alert.AlertType.ERROR, "Champs manquants", "Veuillez remplir tous les champs !");
            return;
        }


        if (!name.trim().matches("^[A-Za-zÀ-ÿ\\s]{2,}$") || name.trim().replaceAll("\\s+", "").length() < 2) {
            showAlert(Alert.AlertType.ERROR, "Nom invalide", "Le nom doit contenir au moins 2 lettres alphabétiques.");
            return;
        }
   ...
   void ajouter(ActionEvent event) {
       ...
       if (!name.matches("^[A-Za-zÀ-ÿ\\s]{2,}$") || name.replaceAll("\\s+", "").length() < 2) {
           showAlert(Alert.AlertType.ERROR, "Nom invalide", "Le nom doit contenir au moins 2 lettres alphabétiques.");
           return;
       }
       ...
       if (user_service.emailExists(email)) {
           showAlert(Alert.AlertType.ERROR, "Email déjà utilisé", "Cet email est déjà associé à un autre utilisateur.");
           return;
       }
       ...
       try {
           user_service.add(u);  // Ajoute l'utilisateur à la base de données
           showAlert(Alert.AlertType.INFORMATION, "Succès", "✅ Utilisateur ajouté avec succès !");
